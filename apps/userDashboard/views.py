...
def add_friend(request, user_id):
    ...
    friend_request = Friend.objects.create(
        friend_requester=friend_requester, request_to=request_to, acceptance=0, rejected=0)
...
def add_new(request):
    ...
    user_messages = Message.objects.filter(
        receiver=user).order_by('-created_at')[:3]
    friend_requests = Friend.objects.filter(request_to=user).filter(
        acceptance=0).annotate(count=Count('acceptance'))
    total_unread = Message.objects.filter(receiver=user).filter(
        read_status=0).annotate(count=Count('read_status'))
    context = {
        "user": user,
...
def add_user(request):
    ...
    hash1 = bcrypt.hashpw(
        request.POST['register_pass1'].encode(), bcrypt.gensalt())
    new_user = User.objects.create(email=request.POST['register_email'], first_name=request.POST['register_first_name'],
                                   last_name=request.POST['register_last_name'], pw_hash=hash1, admin_level="normal")
...
def admin_edit_image(request, user_id):
    ...
    if user_update.admin_level == "superadmin" or user_update.admin_level == "admin":
        ...
        if curr_user.admin_level != "superadmin" and user_update.id != request.session['user_id']:
            messages.error(
                request, "You can't change other admins' pictures!", extra_tags='edit_pass_error')
            return redirect(f'/edit/profile/{user_id}')
    user_update.img_link = request.POST['edit_profile_image']
    user_update.save()
    messages.success(request, 'Image updated!', extra_tags='image')
...
def admin_edit_password(request, user_id):
    ...
    if user_update.admin_level == "superadmin" or user_update.admin_level == "admin":
        if curr_user.admin_level == "superadmin" and curr_user.id == 1:
            hash1 = bcrypt.hashpw(
                request.POST['edit_pass1'].encode(), bcrypt.gensalt())
        ...
        if curr_user.admin_level != "superadmin" and user_update.id != request.session['user_id']:
            messages.error(
                request, "You can't change other admins' passwords!", extra_tags='edit_pass_error')
            return redirect(f'/edit/profile/{user_id}')
    hash1 = bcrypt.hashpw(
        request.POST['edit_pass1'].encode(), bcrypt.gensalt())
    user_update.pw_hash = hash1
...
def admin_edit_user(request, user_id):
    ...
    if user_update.admin_level == "superadmin" or user_update.admin_level == "admin":
        ...
        if curr_user.admin_level != "superadmin" or user_update.id != request.session['user_id']:
            messages.error(
                request, "You can't change other admins' information!", extra_tags='edit_error')
            return redirect(f'/edit/profile/{user_id}')
    user_update.email = request.POST['edit_email']
    user_update.first_name = request.POST['edit_first_name']
    user_update.last_name = request.POST['edit_last_name']
...
def comment(request, user_id, message_id):
    ...
    new_comment = Comment.objects.create(
        comment=request.POST['comment_description'], commenter=commenter, recipient=recipient, the_message=message)
...
def delete_user(request, user_id):
    ...
    if remove_user.id == 1 and remove_user.admin_level == "superadmin":
        messages.error(
            request, "You can't delete the head honcho superadmin!", extra_tags="delete_error")
        return redirect('/manage_users')
    if curr_user.admin_level != "superadmin":
        messages.error(request, "You can't delete another admin!",
                       extra_tags="delete_error")
        return redirect('/manage_users')
    if remove_user.admin_level == "superadmin" and remove_user.id != 1:
        messages.error(
            request, "You can't delete another superadmin!", extra_tags="delete_error")
        return redirect('/manage_users')
...
def message(request, user_id):
    ...
    new_message = Message.objects.create(
        message=request.POST['message_description'], sender=sender, receiver=receiver, read_status=0)
...
def profile(request, user_id):
    print('The user profile is: ', user_id)
    user = User.objects.get(id=user_id)
    curr_user = User.objects.get(id=request.session['user_id'])
    new_messages = Message.objects.filter(read_status=0)
    curr_user_requests = Friend.objects.filter(
        friend_requester=curr_user).filter(request_to=user)
    curr_user_request_by = Friend.objects.filter(
        friend_requester=user).filter(request_to=curr_user)
    total_unread = Message.objects.filter(receiver=curr_user).filter(
        read_status=0).annotate(count=Count('read_status'))
    friend_requests = Friend.objects.filter(request_to=curr_user).filter(
        acceptance=0).annotate(count=Count('acceptance'))
...
def register_user(request):
    ...
    new_user = User.objects.create(email=request.POST['register_email'], first_name=request.POST['register_first_name'],
                                   last_name=request.POST['register_last_name'], pw_hash=hash1, admin_level=admin_level)
...
def reject(request, friend_id):
    ...
    messages.error(request, "Declined friend invite",
                   extra_tags='reject_request')
    return redirect('/friend_requests')
...
def search_results(request):
    ...
    print('search_input is: ',search_input)
    ...
    if search_type == True:
        ...
        print('search_query after split: ', search_query)
        if len(search_query) > 2:
            print('Inside first if')
            search3 = "Invalid search type! Only enter a first and last name."
            ...
            search1 = User.objects.filter(
                first_name=search_query[0], last_name=search_query[1])
            search2 = User.objects.filter(
                first_name=search_query[1], last_name=search_query[0])
